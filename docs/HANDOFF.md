...
| Branded social preview image and metadata | Implemented on `social-preview`; pending review/merge | Static 1200 × 630 PNG, editable SVG source, complete OG image metadata, and `twitter:card=summary_large_image`. Local tests/build and all-six-page metadata audit pass. |
...
**Next task:** review the `social-preview` PR and Pages preview, then merge when approved. The implemented card uses the site's warm neutral palette and typography with “Xiao Nan”, “Applied AI · ML Systems · Product”, and “xiaonan.co”. Source/export instructions are in [design/README.md](../design/README.md). `og:image` and `twitter:image` use absolute production URLs; dimensions/type/alt metadata and `summary_large_image` are included. Page-specific titles/descriptions are preserved.

Acceptance: local tests/build and PR CI pass, Pages preview looks right, the production image returns 200 with the correct content type after merge, and actual social previews are reviewed. Local checks are complete; inspect the branch's latest remote checks for CI/preview results. Production publication and actual platform unfurls remain after owner review. No image service, generator dependency, or runtime JavaScript was added.
