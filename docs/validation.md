...
## Approved publication — 2026-09-20

- Owner explicitly approved merging and publishing the reviewed version, with Work deferred. Final PR head `f4f2418` had successful validation and Pages checks. PR #2 was marked ready and merged with an exact-head guard as `ab6eff25dea57b1d0acd033a56d7ffa8773b659d` at 12:52 UTC (20:52 Asia/Singapore).
- [Main CI 35511850260](https://github.com/xia0nan/xiaonan.co/actions/runs/35511850260) passed. Cloudflare production deployment `4ceb7d6b-6ba8-49ab-ac6c-b4528d566113` succeeded for the merge commit.
- Production HTTPS audit passed for all five public pages, RSS, both sitemap files, robots, and social PNG. Home/About descriptions match the approved preview; current-focus copy and all three profiles are live, and Person `sameAs` matches the shared profile list. Work remains intentionally empty.
- Original Agoda link/date and exactly one RSS item are preserved; sitemap contains exactly Home/About/Work/Writing/Projects; PNG is byte-identical to the committed asset; missing and unpublished draft routes return 404 with noindex. Temporary detailed results: `/private/tmp/xiaonan-v11-release/result.json`.
- Browser access was unavailable in the publication session, so post-release LinkedIn inspector and hosted visual rechecks are pending. HTTP evidence confirms the new description is live; it does not prove LinkedIn's warning has cleared. Previously recorded iPhone/WhatsApp and remaining keyboard/Safari checks also remain pending.
- The preceding “approval/merge pending” bullets are historical pre-release evidence, superseded by this section. The deferred contribution is still in the roadmap todo; this publication does not claim full V1.1 completion.
