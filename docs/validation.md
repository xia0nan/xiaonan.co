...
- Existing local production preview was reused and the rebuilt homepage opened in Chrome. No new dev server was started. PR CI and Pages preview must be checked against the pushed branch; after merge, check production image delivery and actual social unfurls. Preview metadata intentionally points to production, so unfurling the preview before merge is not the final platform acceptance test.
